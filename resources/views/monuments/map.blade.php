...
@section('content')
    <!-- Loading Spinner -->
    <div id="loadingSpinner" class="fixed inset-0 bg-white bg-opacity-90 flex items-center justify-center"
        style="z-index: 99999;">
        <div class="text-center">
            <div class="relative inline-block mb-4" style="height: 120px; width: 120px;">
                <img src="/wlm-logo.svg" alt="WLM Turkey"
                    class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-14 w-14" style="z-index: 2;">
                <div class="absolute inset-1 rounded-full border-8 border-blue-200"></div>
                <div
                    class="absolute inset-0 rounded-full border-8 border-blue-600 border-t-transparent animate-spin shadow-md">
                </div>
            </div>
    ...
    <div class="fixed inset-0">
        ...
        <div id="searchPanel"
            class="absolute top-4 left-4 z-20 bg-white rounded-lg shadow-lg p-4 w-80 max-h-[calc(120vh-8rem)] overflow-y-auto hidden md:block">
            <h3 class="text-lg font-semibold mb-4">Anıt ara <i class="cc-icon cc-cc text-gray-500"></i><i
                    class="cc-icon cc-by text-gray-500"></i><i class="cc-icon cc-sa text-gray-500"></i></h3>

            <!-- Search Input -->
            <div class="mb-4">
                <input type="text" id="searchInput" placeholder="Anıt adı veya açıklama ara..."
            ...
            <div class="space-y-3">
                ...
                <div>
                    ...
                    <input type="range" id="distanceFilter" min="5" max="100" value="50" class="w-full">
            ...
            <div class="mt-4 flex gap-2">
                ...
                <button id="clearFiltersBtn"
                    class="flex-1 bg-gray-100 text-gray-800 px-4 py-2 rounded-md hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-gray-500 hidden">
                    Filtreleri Temizle
                </button>
            </div>
        ...
        <div class="floating-nav-cards absolute top-4 right-4 z-30 flex flex-col gap-3">
            <a href="https://commons.wikimedia.org/wiki/Commons:Wiki_Loves_Monuments_2025_in_Turkey/tr" target="_blank"
                class="bg-white/90 backdrop-blur rounded-xl shadow-md px-4 py-3 hover:bg-white">
        ...
        <div id="monumentInfo"
            ...
            <div id="photoCarousel" class="mb-3 hidden">
                <div class="relative">
                    ...
                    <!-- Carousel Navigation -->
                    <button id="prevPhoto"
                        class="absolute left-2 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white rounded-full p-1 hover:bg-opacity-75">
                    ...
                    <button id="nextPhoto"
                        class="absolute right-2 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white rounded-full p-1 hover:bg-opacity-75">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            @auth
                <div class="mt-3">
                    <button id="quickUploadBtn" onclick="quickUpload()"
                        ...
                        <svg class="w-5 h-5 mr-2" fill="currentColor" viewBox="0 -960 960 960">
                            <path
            ...
            <!-- Upload Wizard Link -->
            <div class="mt-3">
                <a id="monumentUploadWizardLink" href="#"
                    @guest
data-auth="false"
               @else
               target="_blank"
               data-auth="true" @endguest
        ...
        <div class="absolute bottom-4 right-4 z-30 flex flex-col items-end gap-3"
            style="z-index: 999 !important; bottom: 48px;">
            ...
            <button id="mapLocateButton"
                class="bg-white/90 backdrop-blur rounded-full shadow-md hover:bg-white text-gray-700 w-12 h-12 flex items-center justify-center"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function getFilteredMarkers() {
                ...
                if (categoryRaw) {
                    ...
                    if (catQid) {
                        result = result.filter(m => ((m.monument.category_qid || m.monument.type_qid || '')
                            .toUpperCase()) === catQid);
                    } else if (catLabel) {
                        result = result.filter(m => ((m.monument.category_label_tr || m.monument.type_label_tr ||
                            '').toLowerCase().includes(catLabel)));
                    }
                }
                ...
                fetch(url)
                    ...
                    .then(data => {
                        ...
                        (data || []).forEach(item => {
                            ...
                            const marker = L.circleMarker([item.coordinates.lat, item.coordinates
                                .lng
                            ], {
                                radius: finalRadius,
                                fillColor: '#3B82F6',
            ...
            function showMonumentInfo(monument) {
                ...
                if (isAuthenticated) {
                    ...
                    const loginUrl = '{{ route('auth.login') }}' + '?return_url=' + encodeURIComponent(
                        monumentDetailUrl);
            ...
            function setupPhotoCarousel(photos) {
                ...
                photos.forEach((photo, index) => {
                    ...
                    let isPublicDomain = (license && (license.toLowerCase().includes('public domain') ||
                    ...
                    if (overlayText) {
                        if (photo.commons_url) {
                            overlayHtml =
                                `<a href="${photo.commons_url}" target="_blank" class="photo-overlay" style="position:absolute;bottom:0.4em;right:0.4em;pointer-events:auto;">${overlayText}</a>`;
                        } else {
                            overlayHtml =
                                `<div class="photo-overlay" style="position:absolute;bottom:0.4em;right:0.4em;pointer-events:none;">${overlayText}</div>`;
                        }
                    }
            ...
            `;
                    ...
                    indicator.className =
                        `w-2 h-2 rounded-full ${index === 0 ? 'bg-blue-600' : 'bg-gray-300'}`;
            ...
            function goToSlide(index) {
                ...
                indicators.forEach((indicator, i) => {
                    indicator.className =
                        `w-2 h-2 rounded-full ${i === index ? 'bg-blue-600' : 'bg-gray-300'}`;
                });
        ...
        `;
            ...
            function attachCarouselListeners() {
                ...
                console.log('Attaching carousel listeners:', {
                    nextBtn,
                    prevBtn
                });
            ...
            function buildUploadWizardUrl(monument) {
                ...
                if (monument.wikidata_id) {
                    ...
                    description += '\n\{\{Load via app WLM.tr|year=' + new Date().getFullYear() +
                        '|source=wizard\}\}';
        ...
        `;
            ...
            async function uploadPhoto() {
                ...
                if (!form || !button || !errorDiv || !errorMessage || !successDiv || !successMessage) {
                    ...
                    alert('Yükleme formu bulunamadı. Lütfen sayfayı yenileyip tekrar deneyin.');
                    return;
                }
                ...
                try {
                    ...
                    if (!contentType || !contentType.includes('application/json')) {
                        ...
                        throw new Error(
                            'Sunucu beklenmeyen bir yanıt döndürdü. Lütfen giriş yapıp tekrar deneyin.');
                    }
                    ...
                    if (response.ok && result.success) {
                        ...
                        if (result.data?.descriptionurl) {
                            ...
                            if (result.data?.filename) {
                                const commonsUrl =
                                    `https://commons.wikimedia.org/wiki/File:${encodeURIComponent(result.data.filename)}`;
                        ...
                        // Try to show error in modal, fallback to alert
                    ...
                    const errorText = error.message || 'Bir hata oluştu. Lütfen tekrar deneyin.';
